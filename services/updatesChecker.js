...
async function checkForUpdates() {
  ...
  for (let i = 0; i < leads.length; i++) {
    try {
      ...
      if (!msgLead) continue;
      const contact = await crmdataService.getContactById(
        leads[i]._embedded.contacts[0].id
      );
      const crmLead = collectDataFromLead(leads[i], contact);
      if (msgLead.updated_at > crmLead.updated_at) continue;

      for (const prop in crmLead) {
        if (valuesToCheck.includes(prop)) {
          if (crmLead[prop] != msgLead[prop]) {
            console.log(
              "Comparing crmLead & msgLead: not equeal: ",
              crmLead.lead_id,
              prop,
              crmLead[prop],
              msgLead[prop]
